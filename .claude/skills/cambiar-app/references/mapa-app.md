...
- Bienvenida (sección `BIENVENIDA`, instalación nueva): `comprobarBienvenida()` se llama en INIT tras `loadDashboard()`; abre `modal-bienvenida` solo si no está descartada (`localStorage['kmalumnos_bienvenida_descartada']`), sin cuenta conectada (`getEstadoCuenta`) y sin datos locales (`getResumen` con vehiculos/alumnos/practicas a 0). Sus 3 botones: `bienvenidaCrearEmpresa()`/`bienvenidaIniciarSesion()` (cierran bienvenida y abren el modal correspondiente) y `continuarSinCuenta()` (marca el flag y cierra).
