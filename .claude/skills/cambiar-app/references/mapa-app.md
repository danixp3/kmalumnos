...
**Secciones (anclas):** `DIÁLOGOS NATIVOS (fix de foco)`, `ESTADO`, `NAVEGACIÓN`, `DASHBOARD`, `PESTAÑAS DE PÁGINA (Kilómetros / Datos)`, `VEHÍCULOS`, `PROFESORES`, `ALUMNOS`, `PRÁCTICAS`, `PAGOS`, `IMPORTAR CSV`, `EXPORTAR / COMPARAR CSV`, `MODALES`, `UTILS`, `PREFERENCIAS (rango de km por defecto)`, `SOLAPAMIENTOS`, `LOGS`, `BACKUP`, `TIMELINE DEL VEHÍCULO`, `SYNC UI`, `CUENTA DE EMPRESA (antes CREDENCIALES DE SINCRONIZACIÓN)`, `AJUSTES`, `AUTO-UPDATE`, `TOASTS (mensajes no bloqueantes)`, `REGISTRO RÁPIDO`, `BIENVENIDA (instalación nueva)`, `INIT` (todas con prefijo `// ─── `).
...
- Pagos (página propia `page-pagos`, grupo Datos, pestañas Deudas/Tarifas vía `cambiarTabPagos(tab)`): pestaña Deudas — `loadDeudas` pinta `getDeudas()` (permiso, nº prácticas, total generado, pagado, saldo con clase `saldo-pendiente`/`saldo-ok`, aviso `sin_tarifa` por fila y banner `#pagos-aviso-sin-tarifa` si alguna deuda lo tiene); `abrirModalPago`/`openEditPago`/`savePago` (modal `#modal-pago`, cantidad/fecha por defecto hoy/nota); `abrirHistorialPagos`/`deletePagoUI` (modal `#modal-historial-pagos`, borrado con `confirm()`). Pestaña Tarifas — `loadTarifas` construye la rejilla permiso × tipo (permisos = los de alumnos ∪ los ya tarificados, ordenados) con un input editable por celda; `guardarTarifaUI(permiso, tipo, inputId)` llama a `setTarifa` y confirma con `showToast`.
