...
| `generar-km.js` | 210 | Sección "Generar km": encadenado / hasta máximo / por rango (previsualizar→aplicar) |
...
- Página **Generar km** (`generar-km.js`, sección propia del sidebar): selector `#gk-vehiculo` compartido + pestañas `cambiarTabGenerarKm(modo)` (`'encadenado'`|`'maximo'`|`'rango'`). Encadenado aplica directo (`rellenarKmMasivo`); máximo y rango **previsualizan** (`generarKmHastaMaximo`/`generarKmPorRango` con `aplicar=false`) y luego **aplican lo mostrado** con `aplicarPlanKm(vid, asignaciones)` (WYSIWYG, imprescindible porque los modos usan `Math.random`). `loadGenerarKm()` se llama desde `estado.js`. El botón "Generar km" de la ficha del alumno y el rango del import CSV NO se tocan (siguen donde estaban).
...
| `km-algoritmos.js` | 500 | Solapamientos, relleno masivo, corrección, `generarKmHastaMaximo`/`generarKmPorRango`/`aplicarPlanKm` |
