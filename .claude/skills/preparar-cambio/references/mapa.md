...
La ventana de escritorio es `frame: false` (sin marco nativo): la barra de título la pinta `index.html`/`renderer.js` (`#titlebar`), con los canales IPC `ventana-minimizar/-maximizar/-cerrar/-esta-maximizada` y el evento push `ventana-maximizada`. Varias preferencias de usuario viven en `localStorage`, no en `data.json` (no sincronizan entre PCs): rango km por defecto (`kmalumnos_rango_km`), tarjetas visibles del dashboard (`kmalumnos_dashboard_stats`), tutorial visto por página (`kmalumnos_tutorial_visto`). Ya no existe la clave `kmalumnos_bienvenida_descartada` ni un modo "sin cuenta": desde que la app es SaaS puro, `comprobarBienvenida()` exige cuenta de empresa conectada (`getEstadoCuenta().conectado`, es decir credenciales guardadas y válidas, con o sin internet) para dejar pasar, sin excepción por datos locales previos.

## web-remote/ (Vercel, ES modules — la app usa `require`)

`index.html` (SPA móvil, login email/contraseña de Supabase Auth — ya no hay PIN) + `api/`: `_utils.js` (CORS, `requireAuth` valida `Authorization: Bearer <access_token>` y extrae `empresa_id` del JWT, `getSupabase(token)` pasa ese token a PostgREST), `vehiculos`/`alumnos`/`profesores` (GET), `practica` (POST, crea con km=0,0), `crear-alumno` (POST), `practicas-alumno` (GET historial de un alumno), `historial` (GET últimas 24 h), `cancelar-practica` (POST, soft delete, solo `source='web-remote'`). Envs en Vercel: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SYNC_EMAIL`, `SYNC_PASSWORD` (ya no hay `API_PIN`). Tocar web-remote ⇒ desplegar con /desplegar-web.
