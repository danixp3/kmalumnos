...
- **Versión:** 1.5.0 (2026-07-18) publicada en GitHub (https://github.com/danixp3/kmalumnos/releases/tag/v1.5.0) con auto-update verificado (latest.yml → 1.5.0, instalador HTTP 200); incluye todo el trabajo desde 1.4.0 (renombrado seguro, fix sync de alumnos, alumnos↔profesor, web con login de empresa+perfiles+prácticas por alumno+rediseño visual, fix de secuencias de ids en la nube, vista de morosos en Pagos; detalle en HISTORIAL.md). Los 2 PCs siguen en 1.4.0 hasta que reinicien la app (auto-update). Suite: 109 tests en verde (`npm test`).
...
- **CRÍTICO pendiente (A5):** RLS sigue en `allow_all` — la BD sigue expuesta (repo público + anon key en `sync.js`). El sistema multi-empresa ya está implementado (empresa_id en las 7 tablas, UI de cuenta en Ajustes, web-remote con login de empresa) pero falta cerrar el candado: 1) iniciar sesión en Ajustes → Cuenta de empresa en ambos PCs, 2) verificar sync en ambos, 3) cambiar RLS a `empresa_id = auth.uid()` en las 7 tablas, 4) re-backfill de filas con `empresa_id NULL` creadas entre medias. Requiere acceso físico a ambos PCs; NO ejecutar sin confirmación explícita del usuario.
