...
- **Versión:** 1.4.0 (2026-07-17) publicada y en uso; lote de 8 mejoras cerrado el 2026-07-18 (registro rápido, alumnos, backups, sync inmediata, profesores, pagos, anti-solapamiento — detalle en HISTORIAL.md) aún SIN publicar como release. Esquema de Supabase aditivo (profesores, pagos, tarifas, tipo de práctica): no rompe la 1.4.0 instalada, pero los 2 PCs deben actualizar a la próxima release para ver las novedades. Suite: 82 tests en verde (`npm test`).
...
- **CRÍTICO pendiente (A5):** la BD sigue expuesta: repo público + anon key en `sync.js` + RLS `allow_all`. Falta: meter credenciales de sync en ambos PCs (UI: Backup → Cuenta de sincronización) y cambiar RLS a "solo el usuario de sync autenticado" (por uid). Ojo: privatizar el repo rompería el auto-update, y endurecer RLS antes de que ambos PCs tengan credenciales rompería el sync. El PIN (2004) está commiteado en CHANGELOG-SECURITY.md y CONTEXT.md; el token web es `base64(PIN:timestamp)` sin rate limiting real. Diseño decidido para un sistema multi-empresa (cuenta usuario+contraseña, RLS por uid) que resolvería esto; ejecución pendiente de confirmación del usuario porque requiere coordinar credenciales en los 2 PCs sin romper sync ni auto-update.
