...
_Última actualización: 2026-09-09. El detalle histórico completo está en HISTORIAL.md._

- **Versión:** 1.16.1 (PUBLICADA 2026-09-09, auto-update verificado RELEASE-OK). Hotfix: la 1.16.0 no arrancaba ("Cannot find module './utils-ficheros'") porque `utils-ficheros.js` (que `main.js` requiere) no estaba en `build.files` → añadido. Los PCs que llegaron a instalar la 1.16.0 NO se auto-actualizan (se cierran antes de que corra el auto-updater): necesitan reinstalación manual de la 1.16.1 una vez (los datos en %AppData%\KMAlumnos se conservan). Además, la columna "Ejercicio" de la ficha DGT ahora muestra "2 CLASES" en vez de "2".
- **Ficha oficial DGT "formación práctica" (2026-09-09, v1.16.0):** la app rellena el impreso oficial de la DGT (MOD 12/2016-01-ES) con sus datos. Enfoque: `fichas-dgt.js` (pdf-lib) rellena los **campos de formulario reales** de las plantillas en `assets/fichas/` (no texto superpuesto), aplana cada página y añade continuaciones automáticas (11 clases pág.1 + 32/pág.); la casilla Destreza/Circulación se marca con una X (su estado `/Yes_xxx` no aplana bien). "Ejercicio" siempre "2"; observaciones y firmas en blanco. Mapa nombre→casilla en `assets/fichas/mapa-campos-fichas.json` (derivado por geometría, verificado visualmente). Botón "🖨️ Ficha DGT" en el modal de alumno → `abrirFichaDGT`/`generarFichaDGTUI` (renderer/alumnos.js) → IPC `generar-ficha-dgt` → `db.getDatosFichaDGT`. Datos nuevos que alimentan la ficha (aditivos, sync GATEADO, migraciones `migraciones/2026-09-09_*.sql` **APLICADAS en Supabase el 2026-09-09**, datos intactos: 52 alumnos, 410 prácticas): práctica `hora_inicio`; alumno `primer_apellido/segundo_apellido/codigo_postal/poblacion`; profesor `dni`; y "Datos del centro (DGT)" en Ajustes (localStorage `km_centro_datos`, `getCentroDatos()`). CSV import/export: columnas **opcionales** `hora_inicio` y `profesor` (en blanco/omitibles). Suite **355 tests / 40 suites** en verde + `npm run smoke` OK. Sincroniza entre PCs (columnas ya existen en Supabase).

- **Versión anterior:** 1.15.1 (2026-08-06) — correcciones de interfaz. (1) **Calendario**: al pulsar las flechas de cambio de mes se cerraba solo. Causa: `dpRender()` reescribe el `innerHTML` del popup, dejando el `e.target` del clic fuera del DOM, y el manejador global de 'clic fuera' lo interpretaba como externo. Arreglado en `renderer/datepicker.js` (guarda `e.target.isConnected` + `stopPropagation` en `dpClickPop`) y además ahora se reposiciona al cambiar de mes. Bug PREEXISTENTE (venía de antes de la v1.14.0). (2) **Exámenes**: el desplegable de profesor salía vacío — `abrirNuevaPresentacion` llamaba a `llenarSelectProfesores` sin `await`. (3) **Jornada**: `actualizarBotonFichaje` se dispara en cada tecla y las respuestas podían llegar desordenadas, dejando el botón apuntando a la jornada de otro empleado; ahora descarta respuestas obsoletas (token de secuencia) y `toggleFichaje` reconsulta la jornada abierta antes de fichar en vez de fiarse del `dataset` del botón. (4) **main.js**: `loadFile('index.html')` pasa a ruta absoluta con `path.join(__dirname, ...)` (la relativa se resolvía contra `app.getAppPath()`).
...
- **APLICADO EN SUPABASE (2026-08-06):** las 4 migraciones pendientes se aplicaron en producción y verificadas (datos intactos: 52 alumnos, 381 prácticas, 10 vehículos): `2026-08-06_alumno_libro.sql` (5 columnas), `2026-08-06_alumno_permisos.sql` (columna `permisos` jsonb), `2026-08-06_pago_forma_empleado.sql` (2 columnas), `2026-08-06_cargos.sql` (tabla `cargos` + RLS `empresa_all` restringida a jefe + índice). A partir de ahora libro/permisos/forma_pago-empleado/cargos SÍ se sincronizan entre PCs (la detección gateada en runtime de sync.js encuentra ya las columnas/tabla).
