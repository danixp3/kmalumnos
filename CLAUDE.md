...
- **Ficha oficial DGT "formación práctica" (2026-09-09):** la app rellena el impreso oficial de la DGT (MOD 12/2016-01-ES) con sus datos. Enfoque: `fichas-dgt.js` (pdf-lib) rellena los **campos de formulario reales** de las plantillas en `assets/fichas/` (no texto superpuesto), aplana cada página y añade continuaciones automáticas (11 clases pág.1 + 32/pág.); la casilla Destreza/Circulación se marca con una X (su estado `/Yes_xxx` no aplana bien). "Ejercicio" siempre "2"; observaciones y firmas en blanco. Mapa nombre→casilla en `assets/fichas/mapa-campos-fichas.json` (derivado por geometría, verificado visualmente). Botón "🖨️ Ficha DGT" en el modal de alumno → `abrirFichaDGT`/`generarFichaDGTUI` (renderer/alumnos.js) → IPC `generar-ficha-dgt` → `db.getDatosFichaDGT`. Datos nuevos que alimentan la ficha (aditivos, sync GATEADO, migraciones en `migraciones/2026-09-09_*.sql` **SIN aplicar**): práctica `hora_inicio`; alumno `primer_apellido/segundo_apellido/codigo_postal/poblacion`; profesor `dni`; y "Datos del centro (DGT)" en Ajustes (localStorage `km_centro_datos`, `getCentroDatos()`). CSV import/export: columnas **opcionales** `hora_inicio` y `profesor` (en blanco/omitibles). Suite **355 tests / 40 suites** en verde + `npm run smoke` OK. PENDIENTE para sync entre PCs: aplicar las 3 migraciones en Supabase.
