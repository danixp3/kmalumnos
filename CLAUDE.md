...
- **RUMBO — Pivote a SaaS modular (2026-08-04):** AulaMovil será un SaaS por módulos contratables por empresa. Plan y decisiones en `ROADMAP-SAAS.md`; viabilidad AUES/VeriFactu/firma/pagos en `INVESTIGACION-AUES-VERIFACTU.md` (leer ambos al arrancar tarea nueva). Fase 0 en curso.
- **Base de módulos/entitlements (2026-08-04):** creada la infraestructura de "módulos contratados por empresa" SIN aplicar en Supabase y SIN cambiar comportamiento. `migraciones/2026-08-04_modulos_empresa.sql` (+ROLLBACK): tabla `modulos_empresa` (PK `empresa_id,modulo`), RLS solo SELECT `authenticated` por empresa. `sync.js:getModulosActivos()` (caché, error→modo clásico) + IPC `get-modulos-activos` + preload + `renderer/roles.js:moduloActivo(nombre)` (false en modo clásico; el núcleo NO lo llama aún). Hardening: `buscar_uid_por_email` en la migración de roles ahora exige rol='jefe' estricto (evita enumeración de cuentas). Tests: `tests/modulos.test.js` (6). Suite: **207 tests / 18 suites** en verde. PENDIENTE aplicar en Supabase (ensayo en rama + confirmación del propietario).
- **Portal del alumno v1 — solo lectura (2026-08-05, Fase 0 · Bloque 2):** entregable aditivo, NO rompe la web actual (ningún archivo existente de `web-remote/` modificado). Acceso por "enlace inadivinable" (query `?a=<id>&t=<token>`), sin cuenta de alumno. Nuevos: `web-remote/alumno.html` (vista móvil solo lectura), `web-remote/api/alumno-portal.js` (endpoint anon → RPC, 503 si migración sin aplicar). Migración `migraciones/2026-08-05_portal_alumno.sql` (+ROLLBACK): columna `alumnos.portal_token` + función `portal_alumno_datos(int,text)` SECURITY DEFINER acotada (solo lee si el token coincide; sin enumeración). NO aplicada. Suite 207 en verde. PENDIENTE: (1) aplicar migración en Supabase [espera confirmación], (2) botón en la app de escritorio para generar el enlace (toca sync — pendiente de diseñar con cuidado por el modelo offline-first).
...
- **Versión:** 1.11.0 (release publicada, auto-update verificado). Suite: 201 tests en verde, 17 suites (`npm test`).
